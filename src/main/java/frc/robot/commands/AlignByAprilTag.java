...
public class AlignByAprilTag extends CommandBase {
  ...
  final PIDController speedRotController;
  ...
  final ProfiledPIDController speedController;

  public AlignByAprilTag(
  ...
  public void execute() {
    ...
    if (limelight.getTargetRotation() == LimelightDirections.GRID_SIDE)
      targetPose =
          new Pose2d(
              ...
              Rotation2d.fromDegrees(limelight.getTargetRotation().angle()));
    else
      targetPose =
          new Pose2d(
              new Translation2d(targetX * 1.45, targetY * 1.3),
              Rotation2d.fromDegrees(limelight.getTargetRotation().angle()));
    ...
    rot =
        speedRotController.calculate(
            odometry.getPose().getRotation().getDegrees(), limelight.getTargetRotation().angle());

    double speed = -speedController.calculate(distance, 0);

    if (limelight.getTargetRotation() == LimelightDirections.SUBSTATION_SIDE) speed *= 1;

    // System.out.println("DISTANCE: " + distance);

    Rotation2d direction =
        Rotation2d.fromDegrees(
            180 + angleToTarget - odometry.getHeading() + limelight.getTargetRotation().angle());
