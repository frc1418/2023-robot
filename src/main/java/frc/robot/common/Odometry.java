...
public class Odometry {
  ...
  public void update(SwerveModulePosition[] newPositions) {
    ...
    if (limelight.getIsDetecting() && DriverStation.isTeleop()) {
      if (limelight.getTargetRotation() == LimelightDirections.GRID_SIDE)
        reset(
            new Pose2d(
                new Translation2d(-limelight.getYDistance(), limelight.getXDistance()),
  ...
  public Rotation2d getInclineAngle() {
    ...
    double pow = Math.pow(Math.sin(y) * Math.cos(x), 2);
    double rad =
        (Math.PI / 2)
            - Math.acos(
                Math.sqrt(
                    (Math.pow(Math.cos(y) * Math.sin(x), 2) + pow)
                        / (Math.pow(Math.cos(y), 2) + pow)));
  ...
  public Rotation2d getInclineDirection() {
    ...
    if (y == 0 && x == 0)
      ...
      if (y < 0) return Rotation2d.fromRadians(rad + Math.PI);
